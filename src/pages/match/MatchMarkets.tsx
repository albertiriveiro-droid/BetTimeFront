  ...
  return (
    <div className="match-markets-container">
      <div className="match-header">
        <div className="match-teams">{match.homeTeamName} vs {match.awayTeamName}</div>
        ...
        <div key={market.id} className="market-block">
          ...
          <div className="market-selections">
            {market.selections?.map((s) => (
              <button key={s.id} className="selection-btn">
        ...
        <div key={teamName} className="team-block">
          ...
          {Object.entries(marketsByType).map(([type, markets]) => (
            <div key={type} className="market-block">
              <div className="market-title">{formatPlayerMarketType(type as PlayerMarketType)}</div>
              <div className="market-selections">
                {markets.flatMap(pm =>
                  pm.selections?.map(s => (
                    <button key={s.id} className="selection-btn">
                      <span>{pm.playerName || `Jugador #${pm.playerId}`}</span>
                      <span className="odd">{s.odd}</span>
                    </button>
                  )) ?? []
                )}
