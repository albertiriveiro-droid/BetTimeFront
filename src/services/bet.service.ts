...
import type { Bet, BetCreateDTO, BetOutputDTO } from "../types/bet";
...
export const betService = {
  ...
  getActiveByUser(userId: number): Promise<BetOutputDTO[]> {
    return http.get(`/bet/active/${userId}`).then(res => res.data);
  },

  getFinishedByUser(userId: number): Promise<BetOutputDTO[]> {
    return http.get(`/bet/finished/${userId}`).then(res => res.data);
  },

  getWonByUser(userId: number): Promise<BetOutputDTO[]> {
    return http.get(`/bet/won/${userId}`).then(res => res.data);
  },

  getLostByUser(userId: number): Promise<BetOutputDTO[]> {
